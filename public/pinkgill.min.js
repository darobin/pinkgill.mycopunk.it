...
// client/store/ui.js
var $uiTileOverlayOpen = atom(true);
...
var PinkgillUpload = class extends h3 {
  ...
  async handleDrop(ev) {
    ...
    const handles = await Promise.all(
      [...ev.dataTransfer.items].filter((item) => item.kind === "file").map((item) => supportsFileSystemAccessAPI ? item.getAsFileSystemHandle() : item.webkitGetAsEntry())
    );
    handles.forEach((h4) => console.warn(`[${h4.kind === "directory" || h4.isDirectory ? "D" : "F"}] ${h4.name}`));
  }
  render() {
    return ke`<div class="drop"
