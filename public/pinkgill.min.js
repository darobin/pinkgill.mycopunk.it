...
function getCachedManifest(uri) {
...
var $timeline = atom([]);
var $timelineLoading = atom(true);
var $timelineError = atom(false);
...
// client/el/timeline.js
var PinkgillTimeline = class extends (0, import_lit121.withStores)(h3, [$timeline, $timelineError, $timelineLoading]) {
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    if ($timelineLoading.get()) return ke`<div class="loading"><pg-loading></pg-loading></div>`;
    const tiles = $timeline.get();
    if (!tiles?.length) return ke`<span class="empty-timeline">Nothing to show.</span>`;
    return tiles.map((tile) => ke`<pg-tile .tile=${tile.uri}></pg-tile>`);
  }
};
...
var PinkgillTile = class extends h3 {
  ...
  #controller = new import_lit123.MultiStoreController(this, [
    $installs,
  ...
  render() {
    ...
    if (loading) return this.renderCardContainer(ke`<pg-loading></pg-loading>`);
    let dynHeight;
    const manifest = this.#storeData.$manifest.get();
    ...
    return be(
      (async () => {
        ...
        if (isInstallable(this.tile)) {
          ...
          const loading2 = this.#installerData.$installLoading.get();
          footer = ke`<div slot="footer">
            ${error ? ke`<span class="error">${error}</span>` : D}
            ${ke`<sl-icon-button name="bookmark-plus" @click=${this.handleInstall} ?disabled=${loading2} label="Install"></sl-icon-button>`}
          </div>`;
        }
...
var $activeWish = atom({});
async function grantWish(wish, tileURI) {
