...
function getPagePath(router, name, params) {
  const route = router.routes.find((i4) => i4[0] === name);
  const path = route[3].replace(/\/:\w+\?/g, (i4) => {
    let param = params ? params[i4.slice(2).slice(0, -1)] : null;
...
onMount($timeline, async () => {
  sse_default.addEventListener("new-tile", async () => {
    await refreshTimeline();
  });
  sse_default.addEventListener("deletion-change", async () => {
    await refreshTimeline();
  });
});
...
onMount($installs, async () => {
  ...
  sse_default.addEventListener("install-change", async () => {
    await refreshInstalls();
  });
});
...
var PinkgillTileViewer = class extends h3 {
  ...
  render() {
    ...
    return ke`<pg-tile .tile=${data}></pg-tile>`;
  }
};
...
var PinkgillTile = class extends h3 {
  ...
  renderContainer(content, footer) {
    ...
    const manifest2 = this.tile?.manifest;
  ...
  render() {
    ...
    let dynHeight;
    const manifest2 = this.tile?.manifest;
    if (manifest2?.sizing?.width && manifest2?.sizing?.height) {
      const { width, height } = manifest2.sizing;
...
var PinkgillWishDialog = class extends h3 {
  ...
  render() {
    ...
    const label = "Post";
    return ke`<sl-dialog label=${manifest.name} ?open=${!!wish} @sl-after-hide=${stopWishing}>
