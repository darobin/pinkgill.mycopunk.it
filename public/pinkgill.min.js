    ...
    return be(
      (async () => {
        ...
        if (isInstallable(this.tile)) {
          ...
          footer = ke`<div slot="footer">
            ${error ? ke`<span class="error">${error}</span>` : D}
            ${ke`<sl-icon-button name="bookmark-plus" @click=${this.handleInstall} ?disabled=${loading} label="Install"></sl-icon-button>`}
          </div>`;
        }
