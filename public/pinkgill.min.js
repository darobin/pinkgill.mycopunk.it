...
var PinkgillTimeline = class extends (0, import_lit121.withStores)(h3, [$timeline, $timelineError, $timelineLoading]) {
  ...
  async handleMessageDispatching(ev) {
    const tile = [...this.shadowRoot.querySelectorAll("pg-tile")].find((t5) => t5.getWindow() === ev.source);
    if (!tile) return;
  ...
  render() {
    ...
    return tiles.map((tile) => ke`<pg-tile .tile=${tile}></pg-tile>`);
  }
};
...
var manifestCache = {};
async function urlForTile(tile) {
    ...
    return be(
      (async () => {
        ...
        return this.renderCardContainer(
          ke`<iframe src=${url} style=${dynHeight ? `--dynamic-height: ${dynHeight}px` : ""}></iframe>`,
          footer
        );
      })()
    );
