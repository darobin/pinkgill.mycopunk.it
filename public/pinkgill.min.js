...
var require_StoreController = __commonJS({
  "node_modules/@nanostores/lit/lib/StoreController.js"(exports) {
    ...
    exports.StoreController = StoreController7;
  }
});
...
function createRouter(routes, opts = {}) {
  ...
  let click = (event) => {
    let link2 = event.target.closest("a");
    if (link2 && event.button === 0 && // Left mouse button
    link2.target !== "_blank" && // Not for new tab
    link2.origin === location.origin && // Not external link
    link2.rel !== "external" && // Not external link
    link2.target !== "_self" && // Now manually disabled
    !link2.download && // Not download link
    ...
    !event.defaultPrevented) {
      ...
      let hashChanged = location.hash !== link2.hash;
      router.open(link2.href);
      if (hashChanged) {
        location.hash = link2.hash;
        if (link2.hash === "" || link2.hash === "#") {
...
function getPagePath(router, name, params, search) {
  ...
  let route = router.routes.find((i4) => i4[0] === name);
  if (true) {
    if (!route[3]) throw new Error("RegExp routes are not supported");
  }
  let path = route[3].replace(/\/:\w+\?/g, (i4) => {
    let param = params && params[i4.slice(2, -1)];
    if (param) {
      return "/" + encodeURIComponent(param);
    } else {
      return "";
    }
  }).replace(/\/:\w+/g, (i4) => "/" + encodeURIComponent(params[i4.slice(2)]));
  if (search) {
...
// client/api.js
var PolypodAPI = class {
  constructor() {
  }
  async query(what, params) {
    try {
      const r7 = await fetch(`/xrpc/space.polypod.${what}${this.queryString(params)}`);
      if (!r7.ok) return extractError(r7);
      return { ok: true, status: r7.status, data: await r7.json() };
    } catch (err) {
  ...
  async procedure(what, params, body) {
    ...
    const binaryDataTypeNames = ["Blob", "File", "ArrayBuffer"].map((n5) => `[object ${n5}]`);
    try {
      const headers = {};
      if (binaryDataTypes.find((t5) => body instanceof t5) || binaryDataTypeNames.find((n5) => Object.prototype.toString.call(body) === n5)) {
  ...
  queryString(params) {
    ...
    if (params && typeof params === "object") {
      ...
      Object.entries(params).forEach(([k2, v2]) => {
        if (Array.isArray(v2)) v2.forEach((val) => u2.append(k2, val));
        else u2.append(k2, v2);
      });
      q2 = `?${u2.toString()}`;
    }
    return q2;
...
async function extractError(r7) {
  const ret2 = { ok: false, status: r7.status };
  try {
    const data = await r7.json();
    if (data.message) {
      ret2.error = data.message;
      return ret2;
    }
  } catch (err) {
  }
  ret2.error = r7.statusText || `Unknown error ${r7.status}`;
  return ret2;
}
var client = new PolypodAPI();
var api_default = client;

// client/store.js
var resourceDefaults = { loading: false, error: false, status: 0, available: false, dirty: false, data: null };
var Resource = class {
  #store;
  #options;
  constructor(options) {
  ...
  async load(prm) {
    ...
    const r7 = await api_default[this.#options.load](prm);
    this.#store.setKey("status", r7.status);
    if (r7.ok) {
      this.#store.setKey("error", false);
      this.#store.setKey("data", r7.data);
    } else {
      this.#store.setKey("error", r7.error || "Unknown error");
...
function goto(route, params, search) {
  openPage($router, route, params, search);
}
function link(route, params, search) {
  return getPagePath($router, route, params, search);
}
var profile = new Resource({ load: "getCurrentProfile", defaults: { available: true } });
async function loadProfile() {
  await profile.load();
}
var isLoggedIn = computed(profile.store, (p2) => !!p2?.data && !p2.loading);
var actorProfile = new Resource({ load: "getActorProfile" });
...
var currentTile = createTileResource();
function realTileFromCurrent(dr) {
  const real = structuredClone(currentTile.store.get()?.data);
  Object.keys(real).forEach((k2) => {
    if (Array.isArray(real[k2]) && !real[k2].length) delete real[k2];
    if (real[k2] == null) delete real[k2];
  });
  const res = {};
  (real.resources || []).forEach((r7) => {
    res[r7.path] = { src: r7.src, mediaType: r7.mediaType };
  });
  if (!res["/"] && dr && res[dr]) res["/"] = structuredClone(res[dr]);
  real.resources = res;
  return real;
}
function updateCurrentTile(key, value) {
  currentTile.store.setKey(`data.${key}`, value);
}
function addResourceToCurrentTile() {
  const s2 = currentTile.store.get();
  const cur = s2.data?.resources || [];
  currentTile.store.setKey(`data.resources`, [...cur, { path: "/new", src: null, mediaType: null }]);
}
function removeResourceFromCurrentTile(idx) {
  const s2 = currentTile.store.get();
  const cur = [...s2.data?.resources || []];
  cur.splice(idx, 1);
  currentTile.store.setKey(`data.resources`, cur);
}
function addWishToCurrentTile() {
  const s2 = currentTile.store.get();
  const cur = s2.data?.wishes || [];
  currentTile.store.setKey(`data.wishes`, [...cur, { can: null }]);
}
function removeWishfromCurrentTile(idx) {
  const s2 = currentTile.store.get();
  const cur = [...s2.data?.wishes || []];
  cur.splice(idx, 1);
  currentTile.store.setKey(`data.wishes`, cur);
}
function validateCurrentTile(dr) {
  const tile = realTileFromCurrent(dr);
  const report = { count: 0, errors: {} };
  const error = (k2, msg) => {
    report.count++;
    if (!report.errors[k2]) report.errors[k2] = [];
    report.errors[k2].push(msg);
  };
  ...
  if (tile.icons) {
    const { src: src2 } = tile.icons[0] || {};
    if (!src2) error("icons[0]", "No source");
    const res = tile.resources[src2];
  ...
  if (tile.sizing) {
    ["width", "height"].forEach((k2) => {
      if (!tile.sizing[k2]) error(`sizing`, `${k2} is required and must be positive`);
      if (!/^\d+$/.test(tile.sizing[k2])) error(`sizing`, `${k2} must be a positive number`);
    });
  }
  const canSet = /* @__PURE__ */ new Set(["instantiate"]);
  if (tile.wishes) {
    tile.wishes.forEach((w2, idx) => {
      if (!w2.can) error(`wishes[${idx}]`, `can is required`);
      if (w2.can && !canSet.has(w2.can)) error(`wishes[${idx}]`, `${w2.can} is not a valid can value`);
    });
  }
  if (!Object.keys(tile.resources || {}).length) error("resources", "Required");
  Object.entries(tile.resources || {}).forEach(([k2, r7]) => {
    if (!/^\//.test(k2)) error(`resources`, `Path "${k2}" needs to start with a '/'"`);
    if (!r7.mediaType) error(`resources`, `Media type required for "${k2}"`);
    if (!r7.src?.$link) error(`resources`, `Source required for "${k2}"`);
  });
  if (Object.keys(tile.resources || {}).length && !tile.resources["/"]) error("default_resource", "A default resource must be specified");
  return report;
}
async function saveCurrentTile(dr) {
  const tile = realTileFromCurrent(dr);
  const r7 = await api_default.uploadTile({ tile });
  if (r7.ok) {
...
var profileSet = /* @__PURE__ */ new Set(["profile", "tile"]);
var tileSet = /* @__PURE__ */ new Set(["edit", "tile"]);
...
// client/el/404.js
var Pinkgill404 = class extends h3 {
  static styles = i`
    :host {
      display: block;
...
// client/el/nav.js
var import_lit114 = __toESM(require_lib(), 1);

// client/lib/nav-click.js
function navigationalClickHandler(ev) {
...
// client/el/nav.js
var PinkgillNav = class extends h3 {
  #loggedIn = new import_lit114.StoreController(this, isLoggedIn);
  #router = new import_lit114.StoreController(this, $router);
  static styles = [
    i`
      :host {
        display: block;
...
var PinkgillAvatar = class extends h3 {
  #profile = new import_lit116.StoreController(this, profile.store);
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    const p2 = this.#profile.value;
    if (!p2 || !p2.available) return D;
    const { loading, error, data } = p2;
    if (loading) return ke`<div class="avatar"><pg-loading></pg-loading></div>`;
    if (error) return ke`<div class="avatar"><sl-button href="/login" @click=${navigationalClickHandler} data-route="login">login</sl-button></div>`;
    return ke`<sl-dropdown placement="bottom-end">
...
// client/el/loading.js
var PinkgillLoading = class extends h3 {
  static styles = i`
    :host {
      display: block;
      width: 100%;
...
// client/el/login.js
var PinkgillLogin = class extends h3 {
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    return ke`<sl-card>
      ...
      <form action="/api/login" method="post">
        <sl-input name="handle" placeholder="Enter your handle (e.g. alice.bsky.social)" value=${to(handle)} required></sl-input>
        ${ret2 ? ke`<input type="hidden" name="return" value=${ret2}>` : D}
...
// client/el/root.js
var import_lit121 = __toESM(require_lib(), 1);

// client/store/identity.js
var $isLoggedIn = atom(false);
var $identity = atom(null);
var $loginLoading = atom(true);

// client/store/router.js
...
var $computedRoute = computed(
  [$router2, $isLoggedIn, $loginLoading],
  (router, isLoggedIn2, loginLoading) => {
    if (loginLoading) return { route: "loading", params: {} };
    if (!isLoggedIn2) return { route: "login", params: {} };
    return router;
  }
);

// client/lib/api-store.js
async function apiToStore($store, url) {
...
// shared/tile-hash.js
function makeTileHasher(crypto2) {
  return async function tileHash2(did, tid) {
    const msgUint8 = new TextEncoder().encode(`${tid}@${did}`);
    const hashBuffer = await crypto2.subtle.digest("SHA-256", msgUint8);
    return base36.encode(new Uint8Array(hashBuffer));
  };
}

// client/store/tiles.js
var defaultTile = { loading: true };
var $curTile = map(defaultTile);
...
var tileHash = makeTileHasher(window.crypto);
var tilesPerWindow = /* @__PURE__ */ new Map();
function getMatchingActiveTile(win) {
  return tilesPerWindow.get(win);
}

// client/el/root.js
var PinkgillRoot = class extends h3 {
  #profile = new import_lit121.StoreController(this, profile.store);
  #router = new import_lit121.StoreController(this, $router);
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    if (loading) return ke`<div class="loading"><pg-loading></pg-loading></div>`;
    if (route === "login") return ke`<div class="login"><pg-login></pg-login></div>`;
    if (route === "profile" || route === "tile") return ke`<pg-profile></pg-profile>`;
    if (route === "new" || route === "edit") return ke`<pg-tile-editor></pg-tile-editor>`;
    return ke`<pg-404></pg-404>`;
  }
};
customElements.define("pg-root", PinkgillRoot);

// client/el/profile.js
var import_lit123 = __toESM(require_lib(), 1);
var PinkgillProfile = class extends h3 {
  #actorProfile = new import_lit123.StoreController(this, actorProfile.store);
  #currentTile = new import_lit123.StoreController(this, currentTile.store);
  #router = new import_lit123.StoreController(this, $router);
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    if (route === "profile") {
      ...
      const { cid, author, tile: { name, icons: icons2 } = {}, createdAt } = this.#currentTile.value?.data || {};
      if (author && author?.handle !== handle) goto(route, { handle, cid });
      const icon = icons2?.[0]?.src?.$link ? `/xrpc/space.polypod.getBlob?cid=${icons2[0].src.$link}` : null;
      return ke`<div class="tile">
        <div class="tile-bar">
          <div class="tile-icon">
...
// client/el/tile-editor.js
var import_lit125 = __toESM(require_lib(), 1);
...
`;
var PinkgillTileEditor = class extends h3 {
  #tile = new import_lit125.StoreController(this, currentTile.store);
  #router = new import_lit125.StoreController(this, $router);
  #profile = new import_lit125.StoreController(this, profile.store);
  #loggedIn = new import_lit125.StoreController(this, isLoggedIn);
  static properties = {
  ...
  willUpdate() {
    ...
    if (res?.find((r7) => r7.path === "/")) this.defaultResource = res.findIndex((r7) => r7.path === "/");
    else if (this.defaultResource == null && res?.find((r7) => r7.path === "/index.html")) {
      this.defaultResource = res.findIndex((r7) => r7.path === "/index.html");
    }
  }
  async handleSave() {
  ...
  render() {
    const loading = this.#profile.value?.loading;
    if (loading) return ke`<pg-loading></pg-loading>`;
    if (!this.#loggedIn.value) goto("login", {}, { return: window.location.pathname });
    const mode = this.#router.value?.route === "edit" ? "edit" : "new";
    const { name, description, background_color, icons: icons2, sizing, wishes, resources } = this.#tile.value?.data || {};
    const dirty = this.#tile.value?.dirty;
    const selectedIcon = icons2?.length ? icons2[0].src : null;
    return ke`<form>
      ...
      <sl-select
        ...
        clearable
        @sl-input=${this.handleFormUpdate}
      >${filterImages(resources).map((k2) => ke`<sl-option value=${k2.path}>${k2.path}</sl-option>`)}</sl-select>
      ...
      <fieldset>
        ...
        >${resources.filter((r7) => /^\/[^/]*$/.test(r7.path)).map((k2, idx) => ke`<sl-option value=${idx}>${k2.path}</sl-option>`)}</sl-select>
        <pg-render-errors .errors=${this.validationReport?.default_resource}></pg-render-errors>
        ${(resources || []).map((r7, idx) => ke`<div class="resource-line">
          <pg-resource-editor name=${`resources[${idx}]`} .value=${r7} @input=${this.handleFormUpdate}></pg-resource-editor>
      ...
      <fieldset>
        ...
        ${(wishes || []).map((w2, idx) => ke`<div class="wish-line">
          <pg-wish-editor name=${`wishes[${idx}]`} .value=${w2} @input=${this.handleFormUpdate}></pg-wish-editor>
...
function filterImages(res) {
  if (!res) return [];
  return res.filter((r7) => /^image\//.test(r7.mediaType));
}
var canVerbs = ["instantiate"];
customElements.define("pg-wish-editor", class extends h3 {
  static properties = {
    name: { type: String },
  ...
  render() {
    return ke`<div>
      ...
      >${canVerbs.map((k2) => ke`<sl-option value=${k2}>${k2}</sl-option>`)}</sl-select>
    </div>`;
  }
});
customElements.define("pg-resource-editor", class extends h3 {
  static properties = {
    name: { type: String },
    value: { attribute: false, state: true }
  };
  ...
  async setFile(f4) {
    return await this.shadowRoot.querySelector("pg-cid-uploader")?.setFile(f4);
  }
  render() {
    return ke`<div class="resource">
      <sl-input
        type="text"
        name="path"
...
customElements.define("pg-cid-uploader", class extends h3 {
  static properties = {
    name: { type: String },
    value: { attribute: false, state: true },
  ...
  async setFile(file) {
    ...
    const cid = toString(await create(CODEC_RAW, buffer));
    const doneReq = await api_default.hasBlob({ cid });
    this.spinning = false;
    if (!doneReq.ok) {
    ...
    if (!alreadyDone) {
      this.uploading = true;
      const res = await api_default.uploadBlob({ cid }, buffer);
  ...
  render() {
    ...
    if (this.error) {
      ...
      if (this.spinning) {
        ...
        forValue = "file";
        body = ke`
          <div class=${Rt({ drop: true, dropping: this.hovering })}
    ...
    return ke`<div class="cid-uploader">
...
function fileToMediaType(file) {
  let type = src_default.getType(file.name) || file.type;
  type = typeFixes[type] || type;
  return type || "application/octet-stream";
}
customElements.define("pg-tile-source-uploader", class extends h3 {
  static properties = {
    // name: { type: String },
    // value: { attribute: false, state: true },
    hovering: { type: Boolean, state: true },
  ...
  async handleFilePick(ev) {
    ...
    if (files.find((f4) => !f4.webkitRelativePath.startsWith(`${dirPath}/`))) {
      this.error = "Not all selected files are in the same directory.";
      return;
    }
    const tree = {};
    files.forEach((f4) => {
      const path = f4.webkitRelativePath.replace(dirPath, "");
      if (ignoreFiles.find((fn) => fn === f4.name)) return;
      tree[path] = f4;
    });
    await this.processTileSource(tree);
  }
  async processTileSource(tree) {
    ...
    ["name", "description", "background_color", "sizing", "wishes"].forEach((k2) => {
      if (manifest[k2]) updateCurrentTile(k2, manifest[k2]);
    });
    const resources = Object.keys(tree).filter((k2) => k2 !== "/manifest.json").map((path) => ({ path, mediaType: null, src: null }));
    ...
    await ed.updateComplete;
    if (manifest.icons) {
      const { src: src2 } = manifest.icons[0] || {};
      if (src2 && tree[src2]) updateCurrentTile("icons", [manifest.icons[0]]);
    }
  }
  handleClearError() {
    this.error = null;
  ...
  render() {
    ...
    if (this.error) {
      ...
      body = ke`
        <div class=${Rt({ drop: true, larger: true, dropping: this.hovering })}
          @dragover=${this.handleDragOver}
          @dragenter=${this.handleDragEnter}
          @dragleave=${this.handleDragLeave}
          @drop=${this.handleDrop}
    ...
    return ke`<div class="tile-uploader">${body}</div>`;
  }
});
...
customElements.define("pg-render-errors", class extends h3 {
  static properties = {
  ...
  render() {
    if (!this.errors?.length) return D;
    return ke`<ul>${this.errors.map((err) => ke`<li>${err}.</li>`)}</ul>`;
  }
});

// client/el/tile-loader.js
var import_lit127 = __toESM(require_lib(), 1);
var MIN_SIZE = 150;
var MAX_SIZE = 3e3;
var PinkgillTileLoader = class extends h3 {
  #tileResource = createTileResource();
  #tile = new import_lit127.StoreController(this, this.#tileResource.store);
  static properties = {
    cid: {},
  ...
  render() {
    ...
    const src2 = `https://${this.cid}.tile.${window.location.host}/.well-known/web-tiles/`;
    console.warn(`in <pg-tile-loader>`, src2, w2, h4);
    return ke`<iframe src=${src2} width=${w2} height=${h4} loading="lazy"></iframe>`;
  }
};
customElements.define("pg-tile-loader", PinkgillTileLoader);

// public/pinkgill.js
var ret = (document.cookie || "").split(/\s*;\s*/).find((it) => it.startsWith("return="))?.split("=")[1];
if (ret) {
...
lit-html/directives/range.js:
   ...
   *)
*/
//# sourceMappingURL=pinkgill.min.js.map
