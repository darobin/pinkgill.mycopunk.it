...
var PolypodAPI = class {
  ...
  async query(what, params) {
    try {
      ...
      if (!r7.ok) return { ok: false, error: r7.statusText, status: r7.status };
...
function removeResourceFromCurrentTile(key) {
  const s2 = currentTile.store.get();
  const cur = s2.data?.resources || {};
  delete cur[key];
  ...
  render() {
    ...
    return ke`<form>
      ...
      <fieldset>
        ...
        -->
        ${Object.keys(resources || {}).sort().map((k2) => ke`<div class="resource-line">
          <pg-resource-editor name=${`resources.${k2}`} .value=${{ ...resources[k2], name: k2 }} @sl-input=${this.handleFormUpdate}></pg-resource-editor>
          <sl-icon-button name="x-square" label="Remove resource" data-name=${k2} @click=${this.handleRemoveResource}></sl-icon-button>
        </div>`)}
        <div class="action">
      ...
      <fieldset>
        ...
        ${(wishes || []).map((w2, idx) => ke`<div class="wish-line">
          <pg-wish-editor name=${`wishes[${idx}]`} .value=${w2} @sl-input=${this.handleFormUpdate}></pg-wish-editor>
