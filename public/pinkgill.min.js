...
var require_withStores = __commonJS({
  "node_modules/@nanostores/lit/lib/withStores.js"(exports) {
    ...
    var MultiStoreController_1 = require_MultiStoreController();
    var withStores3 = (LitElementClass, atoms) => {
    ...
    exports.withStores = withStores3;
  }
});
...
var { I: et2 } = si;
var st2 = (o5) => null === o5 || "object" != typeof o5 && "function" != typeof o5;
...
var SlRelativeTime = class extends ShoelaceElement {
  ...
  render() {
    ...
    if (this.sync) {
      let nextInterval2;
      if (unit === "minute") {
        nextInterval2 = getTimeUntilNextUnit("second");
      } else if (unit === "hour") {
        nextInterval2 = getTimeUntilNextUnit("minute");
      } else if (unit === "day") {
        nextInterval2 = getTimeUntilNextUnit("hour");
      } else {
        nextInterval2 = getTimeUntilNextUnit("day");
      }
      this.updateTimeout = window.setTimeout(() => this.requestUpdate(), nextInterval2);
    }
...
var PinkgillAvatar = class extends (0, import_lit114.withStores)(h3, [$identity]) {
  static styles = [
    i`
      :host {
        display: block;
...
// client/el/create-tile-dialog.js
var import_lit117 = __toESM(require_lib(), 1);
...
// client/store/timeline.js
var defaultTimeline = {
  loading: true,
  error: false,
  data: []
};
var $timeline = map(defaultTimeline);
...
onMount($timeline, () => {
  sse_default.addEventListener("new-tile", refreshTimeline);
  sse_default.addEventListener("deletion-change", refreshTimeline);
  return () => {
    sse_default.removeEventListener("new-tile", refreshTimeline);
    sse_default.removeEventListener("deletion-change", refreshTimeline);
  };
});

// client/store/ui.js
var $uiTileOverlayOpen = atom(false);
function openTileOverlay() {
  $uiTileOverlayOpen.set(true);
}
function closeTileOverlay() {
  $uiTileOverlayOpen.set(false);
}

// client/lib/bodify.js
function bodify(body, fetchConfiguration) {
...
// client/store/tile-uploader.js
var defaultUploadState = { loading: true, done: false, error: false };
var $tileUploader = map(defaultUploadState);
async function uploadTile(body) {
...
// client/el/create-tile-dialog.js
var PinkgillCreateTileDialog = class extends h3 {
  static styles = [
    i`
      sl-dialog {
        --width: 600px;
  ...
  #uiTileOverlay = new import_lit117.StoreController(this, $uiTileOverlayOpen);
  #tileUploader = new import_lit117.StoreController(this, $tileUploader);
  ...
  render() {
    ...
    return ke`<sl-dialog label="Create tile" @sl-request-close=${this.handleOverlayClose} ?open=${overlayOpen} @sl-after-hide=${closeTileOverlay}>
...
// client/el/install.js
var import_lit119 = __toESM(require_lib(), 1);

// client/store/installs.js
var defaultInstalls = {
  loading: true,
  error: false,
  data: []
};
var $installs = map(defaultInstalls);
async function refreshInstalls() {
  await apiToStore($installs, `/api/installed`);
}
var defaultInstall = { loading: false, error: false, done: false, data: null };
function makeInstallStore() {
  return map(defaultInstall);
}
async function operateInstall($store, tile, operation) {
  $store.setKey("loading", true);
...
async function installTile($store, tile) {
  await operateInstall($store, tile, "install");
}
async function uninstallTile($store, tile) {
  await operateInstall($store, tile, "uninstall");
}
function isInstallable(tile) {
  return !!tile?.manifest?.wishes;
}
function isInstalled(tile) {
  return $installs.get()?.data?.find((ins) => ins.uri === tile.uri);
}
onMount($installs, () => {
  refreshInstalls();
  sse_default.addEventListener("install-change", refreshInstalls);
  return () => {
    sse_default.removeEventListener("install-change", refreshInstalls);
  };
});

// client/store/wishes.js
var $activeWish = atom({});
async function grantWish(wish, tile) {
  console.warn(`granting`, wish, tile);
  $activeWish.set({ wish, tile });
}
function stopWishing() {
  $activeWish.set({});
}

// client/el/install.js
var PinkgillInstall = class extends h3 {
  #installStore = makeInstallStore();
  #install = new import_lit119.StoreController(this, this.#installStore);
  static properties = {
  ...
  render() {
    if (!this.tile) return D;
    const loading = false;
    let content = D;
    if (loading) content = ke`<pg-loading></pg-loading>`;
    if (!this.tile.manifest?.wishes) return D;
    content = ke`<sl-details summary=${this.tile.manifest.name}>
      <sl-menu @sl-select=${this.handleWishMenu}>
        ${this.tile.manifest.wishes.map((w2, idx) => {
      let label = "Wish";
      let icon = "magic";
      if (w2.can === "instantiate") {
        label = w2.name || "New";
        icon = "plus-square";
      }
      return ke`<sl-menu-item value=${idx}>
              ${label}
              ${icon ? ke`<sl-icon slot="suffix" name=${icon}></sl-icon>` : D}
            </sl-menu-item>`;
    })}
        <sl-menu-item value="uninstall">
          ${this.confirmUninstall ? ke`
                  <sl-button size="small" variant="primary" @click=${this.handleConfirmUninstall}>Ok</sl-button>
                  <sl-button size="small" variant="danger" @click=${this.handleCancelUninstall}>Cancel</sl-button>` : "Uninstall"}
          <sl-icon slot="suffix" name="x-lg"></sl-icon>
        </sl-menu-item>
      </sl-menu>
    </sl-details>`;
    return ke`<div class="container">${content}</div>`;
  }
};
customElements.define("pg-install", PinkgillInstall);

// client/el/installed-palette.js
var import_lit121 = __toESM(require_lib(), 1);
var PinkgillInstalledPalette = class extends h3 {
  #installs = new import_lit121.StoreController(this, $installs);
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    let content = ke`<li class="no-results">No installed tiles.</li>`;
    const { loading, error, data } = this.#installs.value;
    if (loading) content = ke`<pg-loading></pg-loading>`;
    else if (error) content = ke`<span class="error">${error}</span>`;
    else if (data?.length) content = ke`<ul>${data.map((it) => ke`<li><pg-install .tile=${it}></pg-install></li>`)}</ul>`;
    return ke`<sl-card>
...
// shared/tile-hash.js
function makeTileHasher(crypto) {
...
function makeInstantiationStores() {
...
// client/el/root.js
var PinkgillRoot = class extends (0, import_lit125.withStores)(h3, [$computedRoute, $uiTileOverlayOpen]) {
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    if (route === "loading") return ke`<div class="loading"><pg-loading></pg-loading></div>`;
    if (route === "login") return ke`<div class="login"><pg-login></pg-login></div>`;
    if (route === "home" || route === "tile") return ke`<div class="home">
      <div class="sidebar">
      ...
      <div class="primary">
        ${{
      home: ke`<pg-timeline></pg-timeline>`,
      tile: ke`<pg-tile-viewer></pg-tile-viewer>`
    }[route]}
      </div>
...
var PinkgillTileLoader = class extends h3 {
  ...
  render() {
    if (!this.url) return D;
    const style = {};
    if (this.dynHeight) style["--dynamic-height"] = `${this.dynHeight}px`;
    return ke`<iframe src=${this.url} style=${se(style)} loading="lazy"></iframe>`;
  }
};
customElements.define("pg-tile-loader", PinkgillTileLoader);

// client/el/tile-viewer.js
var import_lit128 = __toESM(require_lib(), 1);
var PinkgillTileViewer = class extends h3 {
  #tile = new import_lit128.StoreController(this, $curTile);
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    if (error) return ke`<div class="error">${error}</div>`;
    if (loading) return ke`<div class="loading"><pg-loading></pg-loading></div>`;
    if (!data) return ke`<span class="empty-timeline">Nothing to show.</span>`;
    return ke`<div>
      <div class="back-nav">
...
var we = class extends $t {
  ...
  update(t5, s2) {
    ...
    let e6 = i4.length;
    this.bt = s2;
    const r7 = this.K, o5 = this.X;
    this.isConnected || this.disconnected();
    for (let t6 = 0; t6 < s2.length && !(t6 > this.wt); t6++) {
      const n5 = s2[t6];
      if (!me(n5)) return this.wt = t6, n5;
      t6 < e6 && n5 === i4[t6] || (this.wt = _e, e6 = 0, Promise.resolve(n5).then(async (t7) => {
        for (; o5.get(); ) await o5.get();
        const s3 = r7.deref();
        if (void 0 !== s3) {
          const i5 = s3.bt.indexOf(n5);
...
// client/el/tile.js
var import_lit130 = __toESM(require_lib(), 1);
...
// client/el/tile.js
var PinkgillTile = class extends h3 {
  #identity = new import_lit130.StoreController(this, $identity);
  #installedTiles = new import_lit130.StoreController(this, $installs);
  #installStore = makeInstallStore();
  #install = new import_lit130.StoreController(this, this.#installStore);
  static properties = {
    tile: { attribute: false },
  ...
  renderContainer(content, footer) {
    ...
    <div class="post">
      ...
      ${footer ? ke`<div class="footer">${footer}</div>` : D}
    </div>
  </section>`;
  }
  getWindow() {
  ...
  render() {
    ...
    return be(
      (async () => {
        ...
        const footer = ke`<div>
          ${installError ? ke`<span class="error">${installError}</span>` : D}
          <sl-button @click=${this.handleReload} size="small">
            <sl-icon slot="prefix" name="arrow-clockwise"></sl-icon>
            Reload
          </sl-button>
          ${installable ? ke`<sl-button @click=${this.handleInstall} ?disabled=${installLoading || installed} size="small">
              <sl-icon slot="prefix" name="bookmark-plus"></sl-icon>
              Install
            </sl-button>` : D}
        </div>`;
        return this.renderContainer(
          ke`<pg-tile-loader .parent=${this} .dynHeight=${dynHeight} .url=${url}></pg-tile-loader>`,
          footer
        );
      })()
    );
...
// client/el/timeline.js
var import_lit132 = __toESM(require_lib(), 1);
var PinkgillTimeline = class extends h3 {
  #timeline = new import_lit132.StoreController(this, $timeline);
  static styles = [
    i`
      :host {
        display: block;
  ...
  render() {
    ...
    if (error) return ke`<div class="error">${error}</div>`;
    if (loading) return ke`<div class="loading"><pg-loading></pg-loading></div>`;
    if (!data?.length) return ke`<span class="empty-timeline">Nothing to show.</span>`;
    return data.map((tile) => ke`<pg-tile .tile=${tile}></pg-tile>`);
  }
};
customElements.define("pg-timeline", PinkgillTimeline);

// client/el/upload.js
var PinkgillUpload = class extends h3 {
  static properties = {
    name: {},
  ...
  render() {
    ...
    if (!paths.length) {
      return ke`<div class=${classes.join(" ")}
    ...
    return ke`<ul class="resources">
      ${paths.sort().map((p2) => ke`<li>${p2}</li>`)}
    </ul>`;
  }
};
...
// client/el/wish-dialog.js
var import_lit135 = __toESM(require_lib(), 1);
var PinkgillWishDialog = class extends h3 {
  static styles = [
    i`
      sl-dialog {
        --width: 600px;
  ...
  #controller = new import_lit135.MultiStoreController(this, [this.#instantiationData.$instanceDone, this.#instantiationData.$instanceLoading, this.#instantiationData.$instanceError, $activeWish]);
  ...
  render() {
    const { wish, tile } = $activeWish.get();
    if (wish?.can !== "instantiate") return D;
    const err = this.#instantiationData.$instanceError.get();
    const label = "Post";
    return ke`<sl-dialog label=${tile.manifest.name} ?open=${!!wish} @sl-after-hide=${stopWishing}>
