...
import { getTimeline, getTile, indexEvent, installTile, uninstallTile, getInstalls, deleteTile } from './db.js';
...
export default async function createRouter (ctx) {
  ...
  router.get('/api/tile/:hash', async (req, res) => {
    ...
    const hyd = await hydrateTiles([tile], agent)
  ...
  router.get('/api/timeline', async (req, res) => {
    ...
    res.send(await hydrateTiles(tiles, agent));
  });
...
async function hydrateTiles (tiles, agent) {
  ...
  const manData = await Promise.all(
    tiles.map(tile => {
      const man = Array.isArray(tile.resources) && tile.resources.find(({ path }) => path === '/manifest.json');
      return man ? readFile(join(BLOB_PATH, man.src.ref.$link)) : Promise.resolve(null);
    })
  );
  tiles.forEach((t, idx) => {
