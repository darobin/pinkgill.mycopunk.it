...
export default function createRouter (ctx) {
  ...
  router.use(async (req, res, next) => {
    ...
    // XXX THIS IS A TEMPORARY HACK, FIX UPSTREAM
    if (/\.html$/.test(req.path) || req.path === '/') resource.mediaType = 'text/html';
    res.type(resource.mediaType);
