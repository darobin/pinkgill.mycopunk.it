...
import { HOST, /*UPLOAD_PATH, BLOB_PATH,*/ IS_PROD, SESSION_PARAMS } from "./config.js";
...
import createXRPCServer from './xrpc.js';
...
export default async function createRouter (ctx) {
  ...
  // ### XRPC ###
  // Set up the XRPC routes.
  const xrpc = await createXRPCServer(ctx);
  router.use(xrpc.router);

  // ### Events ###
  // Notify client of events that it needs to act upon
  ...
  // XXX we probably want to review some of these
  ...
  router.post('/api/login', express.urlencoded({ extended: true }), async (req, res) => {
    ...
    if (!invited.has(handle)) return sendError(res, 'Polypod is invite-only — contact Robin to get in!');
  ...
  router.get('/api/oauth/callback', cookieParser(), async (req, res) => {
    ...
    try {
      ...
      const clientSession = await getIronSession(req, res, SESSION_PARAMS);
    ...
    router.use(cookieParser(), (req, res, next) => {
      if (/ngrok-free\.app/.test(req.hostname)) {
        const search = new URLSearchParams([
          ...
          ['cookie', req.cookies[SESSION_PARAMS.cookieName]],
        ]).toString();
  ...
  // tiles are served from the <CID>.tile.HOST subdomain
  // - All that that domain does is load the static site from /.well-known/web-tiles/.
  // - The worker then connects to the RASL that's on the root server.
  const tileLoader = express.Router();
  // SW at root
  tileLoader.use((req, res, next) => {
    res.setHeader('access-control-allow-origin', '*');
    res.setHeader('service-worker-allowed', '/');
    res.setHeader('content-security-policy', csp);
    next();
  });
  tileLoader.use('/.well-known/web-tiles/', express.static(rel('../tile-loader')));
  router.use(vhost(`*.tile.${HOST}`, tileLoader));
  const originRx = new RegExp(`^https:\\/\\/.+\\.tile\\.${HOST.replace(/\./g, '\\.')}$`, 'i');
  router.use('/.well-known/rasl/', (req, res, next) => {
    const origin = req.header('origin');
    if (originRx.test(origin)) res.setHeader('access-control-allow-origin', origin);
    next();
  });
  router.use(rasl({
    handler: async (cid, method) => {
      console.warn(`RASL: ${method} ${cid}`);
      // We need a DID so that we can ask PDSs for the CID in case we don't have it.
      // Note: we try whatever the first DID we get is, but we could get many, and
      // some could fail. An improvement would be to try several within some limits.
      const did = await ctx.dbWrapper.getDid4Cid(cid);
      try {
        const blob = await getBlob(cid, did, null, method === 'head' ? 'head' : 'stream');
        return (method === 'head') ? blob : { stream: blob };
      }
      catch (err) {
        return false;
      }
  ...
  //   res.header('Content-Security-Policy', `default-src https://tile.${HOST}/${hash}/ https://tile.${HOST}/.well-known/tiles/wishing blob: data: 'unsafe-inline'`);
  ...
  // Add routes here
  router.get(/^\/(?:login|profile|new|edit).*/, (req, res) => {
