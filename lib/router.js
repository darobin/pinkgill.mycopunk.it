...
export default async function createRouter (ctx) {
  ...
  router.post('/api/install', async (req, res) => {
    ...
    try {
      ...
      if (operation === 'install') await installTile(ctx.db, storeData);
      else if (operation === 'uninstall') await uninstallTile(ctx.db, storeData);
    } 
    catch (err) {
      ctx.logger.warn({ err }, 'Failed to write record');
      return res.status(500).send({ ok: false, error: 'Failed to write record' });
  ...
  router.post('/api/instance', async (req, res) => {
    ...
    if (!data || !tile) return res.status(400).send({ ok: false });
    // XXX this may be missing fields that db expects
    const record = {
    ...
    try {
      ...
      await agent.com.atproto.repo.putRecord(putData);
      res.send({ ok: true });
      ctx.logger.info(`Successfully store instance of "${tile}"`);
      const storeData = {
        record,
