...
import { hasLocalBlob, saveBlob } from './blobs.js';
    ...
    'space.polypod.uploadTile',
    {
      ...
      handler: async ({ auth: { agent }, input: { body: tile } }) => {
        ...
        try {
          ...
          const putData = await saveRecord(agent, 'space.polypod.tile', 'space.polypod.tile#record', { cid, tile });
          await ctx.dbWrapper.indexTile(putData);
          return response({
            cid,
            uri: `at://${agent.assertDid}/space.polypod.tile/${putData.rkey}`,
          });
        }
        catch (err) {
          ctx.logger.error({ err }, 'uploadBlob failed');
...
// Returns tid, throws
async function saveRecord (agent, collection, schema, content) {
  ...
  lexiconValidator.assertValidRecord(schema, record); // this might be incorrect, I'm not sure the collection/$type is right
