...
import { indexEvent, deleteEvent, installTile, uninstallTile, deleteTile, undeleteTile } from './db.js';
...
class JetStreamIngester {
  ...
  connect (since) {
    ...
    params.append('wantedCollections', deleteCollection);
      ...
      try {
        ...
        if (commit.type === 'c' || commit.type === 'u') {
          if (commit.collection === tileCollection) await indexEvent(this.db, evt);
            ...
            if (commit.record.operation === 'delete') await deleteTile(this.db, evt);
            else if (commit.record.operation === 'undelete') await undeleteTile(this.db, evt);
          }
        }
        else if (commit.type === 'd') {
          if (commit.collection === tileCollection) await deleteEvent(this.db, { uri });
          else if (commit.collection === installCollection) {
